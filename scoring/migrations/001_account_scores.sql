...
-- active account. Never overwrite — append new rows so the full score history
...
CREATE TABLE IF NOT EXISTS account_scores (
    ...
    -- For INTENT: 0.0–100.0 (higher = stronger buying intent)
    -- For CHURN:  0.0–1.0   (probability of abandonment within 30 days)
...
CREATE INDEX idx_account_scores_account_id  ON account_scores (account_id);
CREATE INDEX idx_account_scores_domain      ON account_scores (account_domain);
CREATE INDEX idx_account_scores_type_time   ON account_scores (score_type, computed_at DESC);
CREATE INDEX idx_account_scores_computed_at ON account_scores (computed_at DESC);

-- Partial index: fast retrieval of latest INTENT score per account
CREATE INDEX idx_account_scores_latest_intent ON account_scores (account_id, computed_at DESC)
    WHERE score_type = 'INTENT';

-- Partial index: fast retrieval of latest CHURN score per account
CREATE INDEX idx_account_scores_latest_churn ON account_scores (account_id, computed_at DESC)
...
CREATE VIEW v_latest_account_scores AS
